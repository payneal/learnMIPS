...
pairs:  .word 5                  # number of pairs
    ...
    #Test that the program works on arrays of several lengths, both even and odd lengths.
        ...
        ori    $2, $0, 2                 # set $2 to 2 for division
        div    $10, $2                   # div size/2
                                   
        mflo   $1                        # quotient is the value of the loop
        ori    $3, $0, 0                 # the count = 0
        ori    $7, $0, 0                   # this will  help get first half of numbers
        or     $8, $0, $10                   # this will  help get 2nd half of numbers
                
        ori    $2, $0, 4                #give $2  the value of 4
        multu  $2, $8                   # since we are dealing with words(4) need to size*4 to get last place
        mflo   $8                       # this should allow one to get last arraay value
        
        # make it so $9  is our pointer to the front of array
        addu   $9, $9, $2               # just 4 bits beeded to move to array[0]
            
        # make it so $23 is our pointer to the back if array
        
        lui    $23, 0x1000              # SET 2ND ARRAY POINTER
        addu   $23, $23, $8             # point $23 to last array position

    loop: 
        beq    $3, $1, done                # go to done if loop count = loop max
        sll    $0, $0, 0         
        
        # here is where the magic happens
        
        lw     $11, 0($9)                   # grap the front half number
        lw     $12, 0($23)                  # grab the back half number
        sll    $0, $0, 0                          #delay
        sll    $0, $0, 0                          #delay

        # swap the two
        sw    $12, 0($9)                    # move front to back
        sw    $11, 0($23)                   # move back to front
        sll   $0, $0, 0                           # delay
        sll   $0, $0, 0                           # delay

        addu   $9, $9, $2                    # add 4 to point to next array element 

        subu   $23, $23,$2                   # sub 4 to point to prev array element 
        ...
        # now go through array to varify correct results

        
        ori    $3, $0, 0                 # count = 0
        addiu  $10, $10, 1               # add one for loop purpose
        lui    $9, 0x1000                # set array pointer 

    loop2: 
        beq    $3, $10, finish                    # if count = length done
        sll    $0, $0, 0                        # bs step
        
        lw     $20, 0($9)                # $10 = size of array
        sll    $0, $0, 0                        # bs step
        sll    $0, $0, 0                        # bs step
        
        addu   $9, $9, $2               # add 4 to point to next array
        addu   $3, $3, 1                # add 1 
        j      loop2                    # do again
    finish:
        sll    $0, $0, 0
